 ...
 * 
 * @see <a href=
 *      "http://prometheus.io/docs/instrumenting/exposition_formats/">Exposition
 *      formats</a>.
 */
public abstract class Collector {
	...
	/**
	 * Like {@link #collect()}, but the result should only contain
	 * {@code MetricFamilySamples} where {@code sampleNameFilter.test(name)} is
	 * {@code true} for at least one Sample name.
	 * <p>
	 * The default implementation first collects all {@code MetricFamilySamples}
	 * and then discards the ones where {@code sampleNameFilter.test(name)}
	 * returns {@code false} for all names in
	 * {@link MetricFamilySamples#getNames()}. To improve performance, collector
	 * implementations should override this method to prevent
	 * {@code MetricFamilySamples} from being collected if they will be
	 * discarded anyways. See {@code ThreadExports} for an example.
	 * <p>
	 * Note that the resulting List may contain {@code MetricFamilySamples}
	 * where some Sample names return {@code true} for
	 * {@code sampleNameFilter.test(name)} but some Sample names return
	 * {@code false}. This is ok, because before we produce the output format we
	 * will call {@link MetricFamilySamples#filter(Predicate)} to strip all
	 * Samples where {@code sampleNameFilter.test(name)} returns {@code false}.
	 *
	 * @param sampleNameFilter
	 *            may be {@code null}, indicating that all metrics should be
	 *            collected.
	 */
	public List<MetricFamilySamples> collect(Predicate<String> sampleNameFilter) {
		...
		List<MetricFamilySamples> remaining = filterSamples(all, sampleNameFilter);
		return remaining;
	}
	...
	/**
	 * Discards the metrics where {@code sampleNameFilter.test(name)}
	 * returns {@code false} for all names in
	 ...
	 *            collected.
	 */
	protected List<MetricFamilySamples> filterSamples(List<MetricFamilySamples> samples, Predicate<String> sampleNameFilter) {
		List<MetricFamilySamples> remaining = new ArrayList<MetricFamilySamples>(samples.size());
		for (MetricFamilySamples mfs : samples) {
	...
	static public class MetricFamilySamples {
		...
		public final List<Sample> samples; // this list is modified when samples
											// are added/removed.
		...
		/**
		 * @param sampleNameFilter
		 *            may be {@code null} indicating that the result contains
		 *            the complete list of samples.
		 * @return A new MetricFamilySamples containing only the Samples
		 *         matching the {@code sampleNameFilter}, or {@code null} if no
		 *         Sample matches.
		 */
		...
		/**
		 * List of names that are reserved for Samples in these
		 * MetricsFamilySamples.
		 * <p>
		 * This is used in two places:
		 * <ol>
		 * <li>To check potential name collisions in
		 * {@link CollectorRegistry#register(Collector)}.
		 * <li>To check if a collector may contain metrics matching the metric
		 * name filter in {@link Collector#collect(Predicate)}.
		 * </ol>
		 * Note that {@code getNames()} always includes the name without suffix,
		 * even though some metrics types (like Counter) will not have a Sample
		 * with that name. The reason is that the name without suffix is used in
		 * the metadata comments ({@code # TYPE}, {@code # UNIT},
		 * {@code # HELP}), and as this name
		 * <a href="https://github.com/prometheus/common/issues/319">must be
		 * unique</a> we include the name without suffix here as well.
		 */
		public String[] getNames() {
			...
			case SUMMARY:
				return new String[] { name + "_count", name + "_sum", name + "_created", name };
			case HISTOGRAM:
				return new String[] { name + "_count", name + "_sum", name + "_bucket", name + "_created", name };
			case GAUGE_HISTOGRAM:
				return new String[] { name + "_gcount", name + "_gsum", name + "_bucket", name };
			case INFO:
		...
		public boolean equals(Object obj) {
			...
			return other.name.equals(name) && other.unit.equals(unit) && other.type.equals(type) && other.help.equals(help) && other.samples.equals(samples);
		}
		...
		public String toString() {
			return "Name: " + name + " Unit:" + unit + " Type: " + type + " Help: " + help + " Samples: " + samples;
		}
		...
		public static class Sample {
			...
			public boolean equals(Object obj) {
				...
				return other.name.equals(name) && other.labelNames.equals(labelNames) && other.labelValues.equals(labelValues) && other.value == value
			...
			@Override
			public String toString() {
				return "Name: " + name + " LabelNames: " + labelNames + " labelValues: " + labelValues + " Value: " + value + " TimestampMs: " + timestampMs;
			}
		}
	}
