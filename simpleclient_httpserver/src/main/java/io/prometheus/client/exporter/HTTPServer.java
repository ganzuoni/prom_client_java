...
public class HTTPServer implements Closeable {
	...
	public static class HTTPMetricHandler implements HttpHandler {
		...
		protected boolean useScrapingContext = true;
		...
		public void setUseScrapingContext(boolean useScrapingContext) {
			this.useScrapingContext = useScrapingContext;
		}

		public boolean isUseScrapingContext() {
			return useScrapingContext;
		}

		@Override
		public void handle(HttpExchange t) throws IOException {
			HttpCollectorScrapingContext scrapingContext = new HttpCollectorScrapingContext(t);
			...
			if ("/-/healthy".equals(contextPath)) {
				...
				scrapingContext.setSampleNameFilter(filter);

				if (useScrapingContext) {
					TextFormat.writeFormat(contentType, osw, registry.metricFamilySamples(scrapingContext));
				} else {
					if (filter == null) {
	...
	/**
	 * We keep the original constructors of {@link HTTPServer} for
	 * compatibility, but new configuration parameters like
	 * {@code sampleNameFilter} must be configured using the Builder.
	 */
	public static class Builder {
		 ...
		 * {@link #withInetSocketAddress(InetSocketAddress)} or
		 * {@link #withHttpServer(HttpServer)}. Default is 0, indicating that a
		...
		/**
		 * Use this hostname to resolve the IP address to bind to. Must not be
		 * called together with {@link #withInetAddress(InetAddress)} or
		 * {@link #withInetSocketAddress(InetSocketAddress)} or
		 * {@link #withHttpServer(HttpServer)}. Default is empty, indicating
		 * that the HTTPServer binds to the wildcard address.
		 */
		...
		/**
		 * Bind to this IP address. Must not be called together with
		 * {@link #withHostname(String)} or
		 * {@link #withInetSocketAddress(InetSocketAddress)} or
		 * {@link #withHttpServer(HttpServer)}. Default is empty, indicating
		 * that the HTTPServer binds to the wildcard address.
		 */
		...
		/**
		 * Listen on this address. Must not be called together with
		 * {@link #withPort(int)}, {@link #withHostname(String)},
		 * {@link #withInetAddress(InetAddress)}, or
		 * {@link #withHttpServer(HttpServer)}.
		 */
		...
		/**
		 * Use this httpServer. The {@code httpServer} is expected to already be
		 * bound to an address. Must not be called together with
		 * {@link #withPort(int)}, or {@link #withHostname(String)}, or
		 * {@link #withInetAddress(InetAddress)}, or
		 * {@link #withInetSocketAddress(InetSocketAddress)}, or
		 * {@link #withExecutorService(ExecutorService)}.
		 */
		...
		/**
		 * Optional: ExecutorService used by the {@code httpServer}. Must not be
		 * called together with the {@link #withHttpServer(HttpServer)}.
		 *
		...
		/**
		 * By default, the {@link HTTPServer} uses non-daemon threads. Set this
		 * to {@code true} to run the {@link HTTPServer} with daemon threads.
		 */
		 ...
		 * {@code sampleNameFilter.test(name)} returns true.
		 * <p>
		 * Use this if the sampleNameFilter remains the same throughout the
		 * lifetime of the HTTPServer. If the sampleNameFilter changes during
		 * runtime, use {@link #withSampleNameFilterSupplier(Supplier)}.
		 */
		 ...
		 * {@code sampleNameFilter.test(name)} returns true.
		 * <p>
		 * Use this if the sampleNameFilter may change during runtime, like for
		 * example if you have a hot reload mechanism for your filter config. If
		 * the sampleNameFilter remains the same throughout the lifetime of the
		 * HTTPServer, use {@link #withSampleNameFilter(Predicate)} instead.
		 */
	...
	/**
	 * Start an HTTP server serving the default Prometheus registry using
	 * non-daemon threads.
	 */
	...
	/**
	 * Start an HTTP server by making sure that its background thread inherit
	 * proper daemon flag.
	 */
	private void start(boolean daemon) {
		if (daemon == Thread.currentThread().isDaemon()) {
			...
			try {
				...
				// This is possible only if the current tread has been
				// interrupted,
				// but in real use cases this should not happen.
				// In any case, there is nothing to do, except to propagate
				// interrupted flag.
	 ...
	 * 
	 * @deprecated renamed to close(), so that the HTTPServer can be used in
	 *             try-with-resources.
	 */
	public void stop() {
		close();
