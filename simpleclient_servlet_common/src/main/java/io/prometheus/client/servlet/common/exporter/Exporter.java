...
/**
 * The MetricsServlet class exists to provide a simple way of exposing the
 * metrics values.
 */
public class Exporter {
	...
	public static final String NAME_USE_SCRAPING_CONTEXT = "use-scraping-context";
	 ...
	 *            programmatically set a {@link SampleNameFilter}. If there are
	 *            any filter options configured in {@code ServletConfig}, they
	 *            will be merged so that samples need to pass both filters to be
	 *            exported. sampleNameFilter may be {@code null} indicating that
	...
	public void init(ServletConfigAdapter servletConfig) throws ServletConfigurationException {
		...
		if (!allowedPrefixes.isEmpty() || !excludedPrefixes.isEmpty() || !allowedNames.isEmpty() || !excludedNames.isEmpty()) {
			SampleNameFilter filter = new SampleNameFilter.Builder().nameMustBeEqualTo(allowedNames).nameMustNotBeEqualTo(excludedNames).nameMustStartWith(allowedPrefixes)
					.nameMustNotStartWith(excludedPrefixes).build();
		...
		String pUseScrapingContext = servletConfig.getInitParameter(NAME_USE_SCRAPING_CONTEXT);
		if (pUseScrapingContext != null) {
			useScrapingContext = Boolean.parseBoolean(pUseScrapingContext);
		}
	}
	...
	public void doGet(final HttpServletRequestAdapter req, final HttpServletResponseAdapter resp) throws IOException {
		...
		ServletCollectorScrapingContext scrapingContext = new ServletCollectorScrapingContext(req);
		...
		try {
			...
			if (useScrapingContext) {
				TextFormat.writeFormat(contentType, writer, registry.metricFamilySamples(scrapingContext));
			} else {
				if (filter == null) {
	...
	public void doPost(final HttpServletRequestAdapter req, final HttpServletResponseAdapter resp) throws IOException {
		doGet(req, resp);
	}
}
